...
export default function Rechner() {
  ...
  const next = () => {
    if (step.key === 'strom' && !form.detail) { setI(4); return; }
    ...
    <div className="wrap py-10 sm:py-14 max-w-2xl">
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, idx) => {
          ...
          const reached = idx <= i;
          return <div key={s.key} className="h-1.5 flex-1 rounded-full overflow-hidden bg-line">
            {reached && <div className="h-full" style={{ background: 'var(--flow)' }} />}
          </div>;
        })}
      </div>

      <div className="flex items-center gap-3 mb-1">
        <span className="inline-grid place-items-center h-10 w-10 rounded-card bg-grass-soft text-grass-deep"><step.icon className="h-5 w-5" /></span>
        <div>
          <div className="eyebrow">Schritt {i + 1} von {form.detail ? 5 : 4}</div>
      ...
      <div className="card p-6 sm:p-7">
        <div key={step.key} className="animate-risein">
          {step.key === 'standort' && (
          ...
          {step.key === 'gebaeude' && (
            <div className="space-y-4">
              <Field label="Wohneinheiten im Haus" hint="So viele Wohnungen gibt es insgesamt.">
                <input type="number" min="2" className="input tnum" value={form.we} onChange={set('we')} />
              </Field>
          ...
          {step.key === 'strom' && (
            <div className="space-y-4">
              <Field label="Dein aktueller Strompreis (ct/kWh)" hint="Steht auf deiner Stromrechnung – der Arbeitspreis pro kWh.">
                <input type="number" step="0.1" className="input tnum" value={form.gvpreis} onChange={set('gvpreis')} />
              </Field>
              <Field label="Dein Jahresverbrauch (kWh, optional)" hint="Ungefähr. Genauer geht es später im Projekt.">
                <input type="number" className="input tnum" value={form.verbrauch} onChange={set('verbrauch')} placeholder="2500" />
              </Field>
              <label className="flex items-start gap-3 mt-2 p-3.5 rounded-card bg-paper-2 cursor-pointer">
                <input type="checkbox" className="mt-1 accent-grass h-4 w-4" checked={form.detail} onChange={set('detail')} />
                <span className="text-[14px] text-ink-soft"><strong className="text-ink">Ich kenne schon mehr Details</strong> (Dach, Ausrichtung, Anlagengröße). Dann wird die Schätzung genauer.</span>
              </label>
            </div>
          )}
          ...
          {step.key === 'detail' && (
            <div className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-3">
                <Field label="Nutzbare Dachfläche (m², optional)"><input type="number" className="input tnum" value={form.dachflaeche} onChange={set('dachflaeche')} placeholder="120" /></Field>
                <Field label="Ausrichtung"><select className="input" value={form.ausrichtung} onChange={set('ausrichtung')}>{ORIENT.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></Field>
              </div>
              <Field label="Verschattung"><select className="input" value={form.verschattung} onChange={set('verschattung')}>{SHADE.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></Field>
              <div className="grid sm:grid-cols-2 gap-3">
                <Field label="Anlagengröße (kWp, optional)" hint="Leer lassen = wir schätzen."><input type="number" className="input tnum" value={form.kwp} onChange={set('kwp')} placeholder={String(suggestKwp({ we: form.we, dachflaeche: form.dachflaeche }))} /></Field>
                <Field label="Investition (€, optional)"><input type="number" className="input tnum" value={form.invest} onChange={set('invest')} placeholder={String(suggestInvest(+form.kwp || suggestKwp({ we: form.we, dachflaeche: form.dachflaeche })))} /></Field>
              </div>
            </div>
          )}
          ...
          {step.key === 'ergebnis' && (
            <div>
              <div className="flex items-center gap-2 text-[13px] text-sun-ink mb-4 p-3 rounded-card bg-sun-soft/60">
                <Info className="h-4 w-4 text-sun-deep shrink-0" />
                Erste Schätzung – angenommen, alle {de(form.we)} Wohnungen machen mit, zu 90 % des Grundpreises.
              </div>
              <div className="grid grid-cols-2 gap-3">
                <Stat label="Anlagengröße" value={`${de(est.p.kwp)} kWp`} tone="grass" />
                <Stat label="Solarstrompreis" value={ct(est.r.solarpreis)} sub="bei 90 % des Grundpreises" />
                <Stat label="Ersparnis je Haushalt" value={`${eur(est.r.tenantSavingsPerHH)}/a`} tone="grass" sub="bei voller Beteiligung" />
                <Stat label="CO₂ vermieden" value={`${de(est.r.co2)} kg/a`} />
              </div>
              <div className="mt-4 p-4 rounded-card border border-line">
                <div className="flex items-center gap-2 text-sm font-semibold text-ink mb-1"><PiggyBank className="h-4 w-4 text-grass-deep" />Für die Eigentümerseite</div>
              ...
              {err && <p className="text-danger text-sm mt-3 text-center">{err}</p>}
            </div>
          )}
        </div>
      </div>
      ...
      {!last ? (
        <div className="flex items-center justify-between mt-5">
          <button onClick={back} disabled={i === 0} className="btn-ghost disabled:opacity-40"><ChevronLeft className="h-4 w-4" /> Zurück</button>
          <button onClick={next} disabled={!canNext()} className="btn-primary">Weiter <ChevronRight className="h-4 w-4" /></button>
        </div>
      ) : (
